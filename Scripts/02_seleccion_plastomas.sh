...
#
# Requisitos previos en el directorio de trabajo:
#   - wcvp_names.csv
#   - wcvp_distribution.csv
#   - lista_neotropical_filtrada.csv   (salida del script en R)
# ======================================================================
# Conteos rápidos de registros (solo informativo)
cut -d '|' -f1 wcvp_names.csv | sort -u | wc -l #-1    #names (1440076)
cut -d '|' -f2 wcvp_distribution.csv | sort -u | wc -l   #-1    #distribution (443347)
cut -d '|' -f1 lista_neotropical_filtrada.csv | sort -u | wc -l   #-1    #filtrado (127743)
  
#Descarga de la DB del NCBI (RefSeq plastid)
...
#Llaves genus_species para plastomas (FASTA RefSeq)
...
#N° de accesiones por tipo (NC, NW, etc.) – solo informativo
cut -f2 plastid_keys.tsv | cut -c1-2 | sort | uniq -c  #NC:15225, NW:9
...
#N° de accesiones por tipo (NC, NW, etc.) – informativo
cut -f7 wcvp_plastid_matches.tsv \  #NC:3151, NW:0
    | cut -c1-2 \
    | sort \
    | uniq -c
    
#Tabla de cuántos plastomas tiene cada taxón (plant_name_id)
...
#Tabla integrada: plastomas disponibles vs. taxones neotropicales por familia
join -t $'\t' -1 2 -2 2 \
